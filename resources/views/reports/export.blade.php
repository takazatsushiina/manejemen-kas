...
<head>
    ...
    <title>Laporan Arus Kas - {{ $validated['start_date'] }} s/d {{ $validated['end_date'] }}</title>
    <style>
        * {
...
<body>
    <div class="header">
        ...
        <p>Periode: {{ \Carbon\Carbon::parse($validated['start_date'])->format('d F Y') }} - {{ \Carbon\Carbon::parse($validated['end_date'])->format('d F Y') }}</p>
    ...
    <div class="summary">
        ...
        <div class="summary-box net">
            <h3>Rp {{ number_format($totalMasuk - $totalKeluar, 0, ',', '.') }}</h3>
            <p>Arus Kas Bersih</p>
        </div>
    </div>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $tx->isMasuk() ? 'Masuk' : 'Keluar' }}</td>
                <td class="text-right {{ $tx->isMasuk() ? 'text-success' : 'text-danger' }}">
