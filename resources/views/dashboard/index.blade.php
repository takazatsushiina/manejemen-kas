...
@section('content')
        ...
        <div class="stat-card">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-emerald-600">Rp {{ number_format($totalMasuk, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="stat-card">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-slate-800">Rp {{ number_format($saldoKas, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="stat-card">
            <div class="flex items-center gap-4">
                ...
                <div>
                    ...
                    <p class="text-xl font-bold text-slate-800">Rp {{ number_format($pettyCashBalance, 0, ',', '.') }}</p>
                </div>
            </div>
        </div>
    </div>
    ...
    @if(auth()->user()->canApproveTransaction() && ($pendingTransactions > 0 || $pendingReconciliations > 0))
            ...
            <div class="flex-1">
                ...
                <p class="text-sm mt-0.5">
                    @if($pendingTransactions > 0)
                        {{ $pendingTransactions }} transaksi menunggu validasi.
                    @endif
                    @if($pendingReconciliations > 0)
                        {{ $pendingReconciliations }} rekonsiliasi menunggu persetujuan.
                    @endif
                </p>
            </div>
            <a href="{{ route('transactions.pending') }}" class="btn-sm btn-warning shrink-0">
                Lihat Sekarang
            </a>
        </div>
    @endif
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
        ...
        <div class="card">
            ...
            <div class="card-body space-y-3">
                ...
                <a href="{{ route('petty-cash.index') }}" class="btn-outline w-full justify-center">
                    <span class="iconify w-5 h-5" data-icon="heroicons:wallet"></span>
                    Kelola Kas Kecil
                </a>
                <a href="{{ route('reports.index') }}" class="btn-outline w-full justify-center">
                    <span class="iconify w-5 h-5" data-icon="heroicons:document-chart-bar"></span>
                    Lihat Laporan
                </a>
                @if(auth()->user()->canApproveTransaction())
                    <a href="{{ route('reconciliations.create') }}" class="btn-outline w-full justify-center">
    ...
    <div class="table-wrapper">
        <div class="card-header flex items-center justify-between">
            ...
            <a href="{{ route('transactions.index') }}" class="btn-sm btn-outline-primary">
                Lihat Semua
            </a>
        </div>
                ...
                <tbody>
                    @forelse($recentTransactions as $tx)
                        <tr>
                            ...
                            <td class="font-semibold {{ $tx->isMasuk() ? 'text-emerald-600' : 'text-red-600' }}">
