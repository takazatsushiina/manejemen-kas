...
<head>
    ...
    <title>{{ $title ?? 'Sistem Manajemen Uang Kas' }}</title>
    ...
    <div class="flex min-h-screen" x-data="{ sidebarOpen: false }">
            ...
            <nav class="flex-1 py-4 overflow-y-auto">
                ...
                @if(auth()->user()->canApproveTransaction())
                    <a href="{{ route('transactions.pending') }}"
                        class="nav-link {{ request()->routeIs('transactions.pending') ? 'active' : '' }}">
                ...
                <a href="{{ route('petty-cash.index') }}"
                    class="nav-link {{ request()->routeIs('petty-cash.*') ? 'active' : '' }}">
                    <span class="iconify w-5 h-5" data-icon="heroicons:wallet"></span>
                    <span>Kas Kecil</span>
                </a>
                @if(auth()->user()->canApproveTransaction())
                    <a href="{{ route('reconciliations.index') }}"
                        class="nav-link {{ request()->routeIs('reconciliations.*') ? 'active' : '' }}">
        ...
        <div class="flex-1 flex flex-col min-w-0">
            ...
            <header
                ...
                <!-- Page Title -->
                <h2 class="text-lg font-semibold text-slate-800 hidden lg:block">{{ $title ?? 'Dashboard' }}</h2>
                                    ...
                                    @if($pendingTransactionsCount > 0)
                                        <a href="{{ route('transactions.pending') }}" 
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-xs text-slate-500">{{ $pendingTransactionsCount }} transaksi menunggu validasi</p>
                                            </div>
                                            <span class="badge-warning text-xs">{{ $pendingTransactionsCount }}</span>
                                        </a>
                                    @endif
                                    ...
                                    @if($pendingReconciliationsCount > 0)
                                        <a href="{{ route('reconciliations.index') }}" 
                                            ...
                                            <div class="flex-1 min-w-0">
                                                ...
                                                <p class="text-xs text-slate-500">{{ $pendingReconciliationsCount }} rekonsiliasi menunggu</p>
                                            </div>
                                            <span class="badge-primary text-xs">{{ $pendingReconciliationsCount }}</span>
                                        </a>
                                    @endif
                                </div>
                                
                                <div class="border-t border-slate-100 px-4 py-2">
                                    <a href="{{ route('transactions.pending') }}" class="text-xs text-primary-600 hover:text-primary-700 font-medium">
                                        Lihat semua →
                                    </a>
                                </div>
                            @else
